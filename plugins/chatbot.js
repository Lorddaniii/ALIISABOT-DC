...
if (/^quién es tu creador|Bot quién te creo|cómo se llama tu creador$/i.test(m.text) && chat.simi && !chat.isBanned) { 
    conn.reply(m.chat,`❤️ mi creador es OFC FRANKLIN,¿lo necesitas para algo si quieres te puedo comunicar con mi creador🙃?`, m)           
}
...
if (/^alisabot-dc$/i.test(m.text) && chat.simi && !chat.isBanned) { 
...
if (/^cuál es tu última versión$/i.test(m.text) && chat.simi && !chat.isBanned) { 
    conn.reply(m.chat,`✨por el momento eh sido actualizado por mi 💖creador OFC FRANKLIN me encuentro en la versión V2.3 😍 estoy muy feliz ya que estoy más actualizado que nunca 😃`, m)
}
